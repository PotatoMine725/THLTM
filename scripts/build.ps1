...
function Build-WithMaven {
    ...
    if (!(Test-Path $clientJar) -or !(Test-Path $serverJar)) {
...
function Build-WithJavac {
    ...
    New-Item -ItemType Directory -Force -Path $sharedClasses,$serverClasses,$clientClasses,$serverStage,$clientStage,$outDir | Out-Null
    ...
    if ($sharedFiles.Count -eq 0 -or $serverFiles.Count -eq 0 -or $clientFiles.Count -eq 0) {
